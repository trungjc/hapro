...
class elp_cls_sendmail
{
	...
	public static function elp_sendmail_cron_trigger($type = "", $subject = "", $content = "", $crondeliveryqueue = array())
	{
		...
		if($elp_cron_adminmail <> "")
		{
			...
			if($wpmail) 
			{
				wp_mail($adminmail, "[Email posts to subscribers Plugin Alert] Cron URL triggered", $elp_cron_adminmail, $headers);
			}
			else
			{
				mail($adminmail ,"[Email posts to subscribers Plugin Alert] Cron URL triggered", $elp_cron_adminmail, $headers);
			}
		}
	}
